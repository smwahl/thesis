...
\subsection{Simple model of super-Earth formation.}
...
Linear extrapolation of $\rho$ and $S$ at constant $P$ = 100 and 400 GPa. For a
constant entropy, fit $log_{10}(\rho)$ and  $log_{10}(T)$ as a function of
$log_{10}(P)$.

Set up a grid of integrated mass $m$ from the center of the planet with constant
spacing, from 0 to the total mass of the planet. Iteratively
calculate the interior profiles until quantity of interest is sufficiently converged.

Compute the radius as a function of $m$.

\begin{equation} \label{eqn:radius}
    r(m) = \left( \frac{3\pi}{4} \int^m_0 dm' \frac{1}{\rho(m')} \right)^{1/3}
\end{equation}
    
Compute the gravity as a function of $r$ using Poisson's equation.
...
where $G$ is the universal gravitational constant. Then converting to a depth $z=R-r$
the hydrostatic pressure is found as 
...
For the converged profile, the total gravitational binding energy of the model planet
is
...
planet with mass $M$ to have a temperature profile that crosses the solvus.
