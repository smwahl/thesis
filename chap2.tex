...
The DFT fram is based on two theorems proven by Hohenberg and Kohn \cite{hk1964} in
...
soon made soluble in practice and by the Kohn-Sham approach \cite{ks1965} making use
...
The Kohn-Sham approch provides a feasible way of determining the exact ground-state
...
\end{aligned}
\end{equation}
where $f_\text{x}(\zeta)=[(1+\zeta)^{4/3}+(1-\zeta)^{4/3}-2]/[2(2^{1/3}-1)]$, $\zeta=(n^\uparrow-n^\downarrow)/n$, and $n=n^\uparrow+n^\downarrow$. For the correlation energy density, the widely used expression is based on parameterization \cite{Perdew81} of accurate quantum Monte Carlo simulations of homogeneous electron gas \cite{Ceperley1980}
...
There has also been active research (Chapter 5 of \cite{Kohanoff-esbook}) on other
