...
material property. These calculations are, therefore, built up from physical constants
...
must be made  to find a tractable solution to the many-body Schr\"odinger equation.
...
pose difficulties for the design and interpretation of experiments. For the
...
is due primarily to the advantage of its better computational efficiency. This
computational efficiency means that DFT can be applied to larger atomic
...
determining stability between phases, in which energy differences are extremely small.
Since the focus of the work presented here is on systems involving the exchange of
atoms between phases, we expect the energy differences to be large enough for DFT
...
The DFT approach is based on two theorems proven by Hohenberg and Kohn
...
The Kohn-Sham formulation for the ground-state energy functional is 
...
homogeneous electron gas. LDA continues to be a popular choice due to its simplicity,
...
schemes of generalized gradient approximation (GGA) have been developed, in which the
exchange-correlation functional is a function of both the density of electrons $n$
and its gradient $\nabla n$. In recent years, one of the most
...
rungs of the Jacob's Ladder, such as meta-GGA, hybrid functionals, toward
...
$NPT$ ensemble (in which the number of atoms, pressure and temperature is conserved). For
...
At its heart, a MD simulation is simply an extension of Newton's laws of motion. Considering the
...
necessary. The new position of an atom  ${\bf r}_{n+1}$ is updated based on its
...
time-averaged temperature over the course of the simulation. This is accomplished through the
...
problems because it is needed  to calculate and compare the Gibb's free
...
The simulations in the table above were performed by Burkhard Militzer. In line 2 of
